...
func (sm *StateManager) NewState() string {
	sm.mu.Lock()
	defer sm.mu.Unlock()
...
func (sm *StateManager) UpdateState(id string, updateFunc func(*State)) {
	sm.mu.Lock()
	defer sm.mu.Unlock()
...
func (sm *StateManager) SetAddress(id, address string) {
	sm.mu.Lock()
	defer sm.mu.Unlock()
	sm.addressToID[address] = id
}

func (sm *StateManager) GetIDByAddress(address string) string {
	sm.mu.Lock()
	defer sm.mu.Unlock()
